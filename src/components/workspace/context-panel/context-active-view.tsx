...
import { FieldDynamics } from "./field-dynamics";
...
export function ContextActiveView({ thread, context }: ContextActiveViewProps) {
  ...
  const influenceField = useInfluenceField();
  ...
  // Field Dynamics presence — determine before computing delays
  const hasFieldDynamics =
    influenceField.fieldAnchorIds.length > 0 ||
    context.unresolvedContinuations.length > 0 ||
    relatedThoughts.some((t) => t.status === "unresolved") ||
    context.semanticRelationships.some((r) => r.kind === "contrasts");
 
  // Staggered delays — field dynamics sits between resonance and concepts
  const adjacentDelay = 0.04;
  const resonanceDelay = hasAdjacent ? 0.1 : 0.04;
  const fieldDynamicsDelay = hasResonance ? 0.16 : hasAdjacent ? 0.14 : 0.06;
  const conceptsDelay = hasFieldDynamics
    ? fieldDynamicsDelay + 0.1
    : hasAdjacent || hasResonance
    ? 0.14
    : 0.04;
  ...
  return (
    ...
    >
      ...
      <motion.section {...contextSectionReveal(conceptsDelay + 0.04)} className="space-y-4">
      ...
      <motion.section {...contextSectionReveal(conceptsDelay + 0.08)} className="space-y-4">
        <h3 className="type-label">Recurring themes</h3>
        <ThemeTags themes={context.recurringThemes} baseDelay={0.14} />
      </motion.section>
 
      <motion.section {...contextSectionReveal(conceptsDelay + 0.12)} className="space-y-4">
      ...
      <motion.section {...contextSectionReveal(conceptsDelay + 0.16)} className="space-y-4">
